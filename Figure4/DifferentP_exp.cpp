...
long double getFitness(vector<char> &point, map<vector<char>, long double> &fitness, long double p) {
    if (fitness.count(point)==0) {
        ...
        if (dis(gen)) {
            ...
            while (distortion > 8.0) distortion = dis2(gen);
...
int main()
{
    ...
    string filename = "DifferentP_04exp8_n="+to_string(n)+"_numIter=" +to_string(numIter) + "_k=" + to_string(k) + "_lambda=" + to_string(lambda) +  ".txt";
